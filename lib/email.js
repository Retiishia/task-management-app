 ...
 * Uses Brevo (formerly Sendinblue) as the SMTP relay provider.
 ...
 *   EMAIL_USER: your Brevo account login email
 *   EMAIL_PASS: your Brevo SMTP Key (generated in Brevo dashboard → SMTP & API)
 */
export async function sendVerificationEmail({ to, name, code }) {
  ...
  if (!user || !pass) {
    ...
    console.log('-----------------------------------------------------------');
    console.log('To send REAL emails via Brevo, add EMAIL_USER and EMAIL_PASS to .env.local');
    console.log('===========================================================\n');
    return { success: true, simulated: true };
  }

  // Brevo SMTP settings (overridable via env vars)
  ...
  try {
    const transporter = nodemailer.createTransport({
      host,
      port,
      secure: false, // STARTTLS — Brevo uses port 587 with STARTTLS (not SSL 465)
      auth: {
        user,
        pass,
      },
      ...
      <div style="font-family: Arial, sans-serif; background-color: #0b0f19; color: #f3f4f6; padding: 40px 20px; border-radius: 16px; max-width: 500px; margin: 0 auto;">
        ...
        <div style="background-color: rgba(30, 41, 59, 0.8); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; padding: 24px; text-align: center;">
          ...
          <p style="color: #cbd5e1; font-size: 14px; line-height: 1.5;">
            Thank you for registering with TaskFlow Pro! Use the 6-digit verification code below to complete your account setup:
          </p>
    ...
    await transporter.sendMail({
      ...
      to,
      subject: `🔑 Your TaskFlow Pro Verification Code: ${code}`,
      html: htmlContent,
    });

    console.log(`✅ [REAL EMAIL SENT] Verification code successfully sent to ${to} via Brevo (${host})`);
